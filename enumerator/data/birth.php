...
include("config.php");
     function signup(){
       ...
       $reg_date=date("y-m-d");
       $sql ="SELECT * from birth WHERE idno = '$idno' AND nationality='$nationality' AND region='$region' AND zone='$zone' AND woreda='$woreda' AND town='$town' AND kebele='$kebele'";
       $user_registration=mysqli_query($obj->conn,$sql) or die(mysqli_connect_error($sql));
       $no_rows = mysqli_num_rows($user_registration);
       if($no_rows == 0)
        {
          $sql2 = "INSERT INTO `birth` (`idno`, `firstname`, `lastname`, `gfname`, `nationality`, `region`, `zone`, `woreda`, `town`, `kebele`, `bdate`,`gender`, `regdate`)
